        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAEOV3JoB3W7cztyOMKe3EJWI1UYj95jygACdxkAAtVGeFfXdEMEhVV3jTYE") 
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/Tv_Serial_Search_Group</code>")    
...
async def set_tutorial_2(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>ᴄᴏᴍᴍᴀɴᴅ ɪɴᴄᴏᴍᴘʟᴇᴛᴇ !!\n\nᴜꜱᴇ ʟɪᴋᴇ ᴛʜɪꜱ -</b>\n\n<code>/set_tutorial_2 https://t.me/MRN_Tutorial</code>")
...
async def set_tutorial_3(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_tutorial https://t.me/MRN_Tutorial</code>")
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://t.me/Tv_Serial_Search_Group').json()
...
async def set_shortner_2(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://t.me/+STRuLEmZ7AgxZjA1').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
